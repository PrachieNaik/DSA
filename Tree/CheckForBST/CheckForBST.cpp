...
Problem statement: Given a binary tree. Check whether it is a BST or not. A binary search tree (BST) is a node based binary tree data structure which has the 
...
Approaches:
    ...
    Method 1: For each node, check if max value in left subtree is smaller than the node and min value in right subtree greater than the node. 
    Write a utility helper function isBSTUtil(struct node* node, int min, int max) that traverses down the tree keeping track of the narrowing min and max allowed 
